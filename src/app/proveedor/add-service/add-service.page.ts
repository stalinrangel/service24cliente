...
export class AddServicePage implements OnInit {
  ...
  public registerServiceForm: FormGroup= new FormGroup({});
	...
	async selectZone() {
		...
		if (this.restricciones.zonas>=this.selecZones.length) {
	...
	public takePicture2(sourceType:any, type:any) {
    ...
    console.log(this.registerServiceForm.value)
		if (this.images_upload.length>0) {
			...
			if (this.images_upload.length > 0) {
				if (true) {
					this.registerServiceForm.patchValue({imagen: this.images_upload[0].url});
						...
						if (items) {
							let items2:any=this.storage.get('TRPSV24');
								if (items2) {
									...
									data => {
										console.log(data);
										this.horario.producto_id=data.producto.id;
										this.enviarHorario();
										this.dismiss();
			...
			this.presentToast('Debe agregar al menos una imagen del servicio');
		}
	}
  ...
  async presentToast(text:any) {
	...
	enviarHorario(){
		...
		this.userService.createHorario(this.horario).subscribe((response:any)=>{
			console.log(response)
