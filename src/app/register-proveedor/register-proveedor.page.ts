...
@Component({
  ...
  templateUrl: './register-proveedor.page.html',
  styleUrls: ['./register-proveedor.page.scss'],
})
export class RegisterProveedorPage implements OnInit {
  ...
  saveR(){
    let zona:any=this.GeneralService.getZone();
    if (zona.id==-1) {
      zona.id=1;
    }
    this.registerUserForm.value.zona_id=zona.id;
    console.log(this.registerUserForm)
    this.presentLoading();
    ...
    this.auth.register(credentials).subscribe({
      next(data){
        ...
        if (data) {
          ...
          //self.navCtrl.navigateRoot('/login');
          setTimeout(() => {
            self.navCtrl.navigateRoot(['login-proveedor']); 
            self.objService.setLoginProveedor(credentials.email);
          }, 1500); 
          self.presentToast('¡Registrado con éxito!');
          //self.loading.dismiss();
        }
